...
def migrate_legacy_knowledge():
    from services.knowledge_manager import kb_manager
    legacy_map = {"history_knowledge": "世界史专家", "epub_knowledge": "EPUB编辑"}
    for collection_name, agent_name in legacy_map.items():
        try:
        ...
        if existing:
            kb_manager.assign_kb_to_agent(agent_name, existing[0], is_default=True)
            continue
        kb = kb_manager.create_kb(
            name=f"{agent_name}默认知识库",
