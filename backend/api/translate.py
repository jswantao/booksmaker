...
async def translate(req: TranslateRequest):
    ...
    agent = get_agent_by_task(task_name)
    if agent is None:
        return {"success": False, "error": f"未知任务类型: {task_name}"}

    terms = memory.get_terminology()
    terms_text = " | ".join(f"{en}→{zh}" for en, zh in list(terms.items())[:12]) if terms else "(无)"
    # Only LongTextTranslator uses context_summary placeholder; passing it to others is harmless (no-op replace)
